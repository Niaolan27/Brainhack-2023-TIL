...
def detect_digits(digit_detection_service, audio_dir: str):
    digits_result = {}
    digits = digit_detection_service.transcribe_audio_to_digits(audio_dir)
    return digits
    #audio_dict = load_audio_from_dir(audio_dir)
...
def main():
    ...
    loc_service = LocalizationService(host='172.16.18.20', port=5577)  # if passthrough sim, use sim's ip address.
    rep_service = ReportingService(host='172.16.18.20', port=5512)
    ...
    # controller = PIDController(Kp=(1, 0.3 ), Ki=(0,  0), Kd=(0.0,0.0))  # this can be tuned.
    controller = PIDController(Kp=(0.3, 0.3, 0.75), Ki=(0, 0, 0), Kd=(0.0, 0.0, 0.0))  # this can be tuned.
    ...
    while True:
        ...
        if try_start_tasks:
            ...
            if try_start_tasks:
                ...
                # submit answer to scoring server and get scoring server's response.vel_cmd
        ...
        while (True):
            ...
            if round(dist_to_goal, 2) <= REACHED_THRESHOLD_M:  # Reached checkpoint.
                ...
                while abs(rel_ang) > 20:
                    ...
                    vel_cmd = controller.update((0, 0, -rel_ang))
                    robot.chassis.drive_speed(x=0,y=0, z=vel_cmd[2])
                    print(f'rot speed: {vel_cmd}')
                ...
                x_diff = (curr_wp[0] - pose[0])
                y_diff = -(curr_wp[1]- pose[1])
                ang_diff = ang_difference(pose[2],  0)
                ...
                vel_cmd = controller.update((x_diff, y_diff, -ang_diff))
                ...
                logging.getLogger('testing').info( f"heading diff: {ang_diff:.2f}")
                logging.getLogger('Control').info(
                    f"input for final speed: {vel_cmd[0]:.2f}, {vel_cmd[1]:.2f}, {vel_cmd[2]:.2f}")
                
                robot.chassis.drive_speed(x=vel_cmd[0], y=vel_cmd[1], z=vel_cmd[2])
                time.sleep(0.5)
            else:
...
if __name__ == '__main__':
    ...
    with open(cfg_path, 'r') as f:
        ...
        if cfg['use_real_models'] == True:
            ...
            from mock_ai_services import MockDigitDetectionService, MockSpeakerIDService, MockObjectReIDService
